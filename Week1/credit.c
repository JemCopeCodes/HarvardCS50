...
int checkSum(long number){
    ...
    while (number > 0){
        ...
        sum += 2*((number/10) % 10) % 10 + 2*((number/10) % 10)/10; //Multiply every other digit by 2, starting with the second-to-last digit, then add those the digits of the product togther
